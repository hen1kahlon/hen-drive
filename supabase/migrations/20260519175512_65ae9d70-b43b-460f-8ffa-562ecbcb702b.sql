...
CREATE OR REPLACE FUNCTION app_private.has_admin_or_editor(_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
SET search_path = public, app_private
AS $$
  SELECT app_private.has_admin_or_editor(_user_id)
$$;
...
REVOKE EXECUTE ON FUNCTION app_private.has_admin_or_editor(uuid) FROM PUBLIC;

GRANT USAGE ON SCHEMA app_private TO anon, authenticated;
GRANT EXECUTE ON FUNCTION app_private.has_role(uuid, public.app_role) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION app_private.has_admin_or_editor(uuid) TO authenticated;
