...
#### מה נחשב "שינוי" שדורש אישור

- כל push לקוד (קומפוננט, routing, סטיילינג, לוגיקה)
- שינוי ב-GitHub Actions workflows
- עדכון `site_settings` ב-Supabase — **כי Supabase משותף לשתי הסביבות**, כלומר שינוי DB נכנס מיד לפרודקשן גם בלי merge
- הוספת תמונות לסטורג' שמוצגות דרך `about_slides` או כל שדה דומה

#### תהליך בקשת שינוי

כשהמשתמש מבקש שינוי חדש, Claude צריך:
1. לבצע את השינוי על `staging`
2. לחכות שה-CI יסיים (כ-2 דקות)
3. לשלוח קישור לסטייג' לבדיקה
4. **לא לגעת ב-`main` ולא ב-Supabase production data** עד שהמשתמש אומר "מאושר"
